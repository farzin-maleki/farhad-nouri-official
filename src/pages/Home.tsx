...
const previews = [
  ...
  { to: "/gallery", label: "Gallery", desc: "Cinematic moments from competitions and training.", icon: ImageIcon, img: galleryPre },
  { to: "/videos", label: "Videos", desc: "Match highlights, workouts and behind the scenes.", icon: Play, img: videosPre },
  { to: "/book", label: "Book a Session", desc: "Train with me — wrestling or fitness, one-on-one.", icon: Calendar, img: bookingPre },
  { to: "/contact", label: "Contact", desc: "For media, sponsorships and collaborations.", icon: Dumbbell, img: contactPre },
];
    ...
    <div className="bg-background text-foreground">
        ...
        <div className="absolute inset-0 pointer-events-none">
          {Array.from({ length: 14 }).map((_, i) => (
            <span
              ...
              style={{ top: `${(i * 37) % 100}%`, left: `${(i * 53) % 100}%`, animation: `float-particle ${6 + (i % 5)}s ease-in-out ${i * 0.4}s infinite` }}
            />
          ))}
        </div>
      ...
      <section className="border-y border-border bg-card/40 py-10">
        <div className="mx-auto max-w-6xl px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          {[{ v: "15+", l: "Years on the mat" }, { v: "30+", l: "Medals" }, { v: "2", l: "Disciplines" }, { v: "100s", l: "Athletes coached" }].map((s) => (
            <div key={s.l}>
