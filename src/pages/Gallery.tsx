...
export default function Gallery() {
  ...
  return (
    <div>
      ...
      {[{ title: "Wrestling", list: wrestling }, { title: "Fitness", list: fitness }].map((sec) => (
        <section key={sec.title} className="px-6 pb-20">
          <div className="mx-auto max-w-7xl">
            ...
            <div className="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
              {sec.list.map((p, i) => (
                <button key={i} onClick={() => setLightbox(p)} className="block w-full overflow-hidden group break-inside-avoid">
                  <img src={p.src} alt={p.alt} loading="lazy" className="w-full h-auto object-cover transition-transform duration-500 group-hover:scale-105" />
                </button>
              ))}
            </div>
          </div>
      ...
      {lightbox && (
        <div className="fixed inset-0 z-[100] bg-background/95 backdrop-blur-md flex items-center justify-center p-6 animate-fade-in" onClick={() => setLightbox(null)}>
          <button aria-label="Close" className="absolute top-6 right-6 text-foreground hover:text-accent" onClick={() => setLightbox(null)}>
            <X className="h-8 w-8" />
          </button>
          <img src={lightbox.src} alt={lightbox.alt} className="max-h-[90vh] max-w-[90vw] object-contain animate-scale-in" onClick={(e) => e.stopPropagation()} />
        </div>
      )}
      <SiteFooter />
    </div>
