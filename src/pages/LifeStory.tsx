...
const timeline = [
  { year: "2008", title: "First Steps on the Mat", text: "At ten years old, walked into a local wrestling club in Tehran. The smell of the mat never left.", img: kidPic },
              ...
              {timeline.map((item, i) => (
                <div key={item.year} className={`relative md:grid md:grid-cols-2 md:gap-12 items-center ${i % 2 === 0 ? "" : "md:[direction:rtl]"}`}>
                  ...
                  <div className="pl-12 md:pl-12 md:[direction:ltr] mt-6 md:mt-0">
                    {item.img && <img src={item.img} alt={item.title} width={800} height={1000} loading="lazy" className="w-full aspect-[4/3] object-cover" />}
                  </div>
                </div>
              ))}
            </div>
