...
type Item = { category: "wrestling" | "fitness"; year: number; comp: string; position: "gold" | "silver" | "bronze"; location: string; };

const items: Item[] = [
  { category: "wrestling", year: 2026, comp: "European Championship Bronze Medalist", position: "bronze", location: "Tirana, Albania" },
  { category: "wrestling", year: 2019, comp: " Asian U23 Championships", position: "silver", location: "Ulaanbaatar, Mongolia" },
  { category: "wrestling", year: 2025, comp: "British Senior Wrestling Championships", position: "gold", location: "UK" },
  { category: "wrestling", year: 2024, comp: "British Senior Wrestling Championships", position: "gold", location: "Manchester, England" },
  { category: "wrestling", year: 2023, comp: "British Senior Wrestling Champion", position: "gold", location: "UK" },
  { category: "wrestling", year: 2022, comp: "English Senior Wrestling Champion", position: "gold", location: "UK" },
        ...
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-wrap justify-center gap-3 mb-14">
            {(["all", "wrestling", "fitness"] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)} className={`px-6 py-3 text-xs uppercase tracking-[0.25em] font-display border transition-all ${filter === f ? "bg-accent text-accent-foreground border-accent" : "border-border text-muted-foreground hover:border-accent hover:text-accent"}`}>
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {list.map((item, i) => (
              <div key={`${item.year}-${item.comp}`} className="group relative border border-border bg-card p-8 hover:border-accent transition-all animate-fade-up" style={{ animationDelay: `${i * 60}ms` }}>
                ...
                <p className={`mt-4 text-xs uppercase tracking-[0.25em] font-display ${colors[item.position].split(" ").pop()}`}>{item.position} medal</p>
              </div>
            ))}
          </div>
        </div>
