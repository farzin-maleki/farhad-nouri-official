...
export const Route = createFileRoute("/book")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Book a 1-on-1 wrestling or fitness session with Farhad Nouri." },
    ],
  }),
  component: Book,
});
...
const faqs = [
  ...
  { q: "What's your cancellation policy?", a: "Free reschedule up to 24 hours before the session. After that, the session is charged." },
];
        ...
        <div className="mx-auto max-w-6xl grid gap-8 md:grid-cols-2">
          {offers.map((o) => (
            <div key={o.title} className="relative border border-border bg-card p-10 hover:border-accent transition-all flex flex-col">
              ...
              <a
                href="#"
                className="group inline-flex items-center justify-center gap-3 bg-accent text-accent-foreground px-6 py-4 font-display tracking-[0.25em] uppercase text-sm hover:bg-accent/90 transition-all"
              >
                Book Now <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
              </a>
              <p className="text-center text-xs text-muted-foreground mt-3">Powered by Cal.com (coming soon)</p>
            </div>
          ))}
        </div>
      </section>
