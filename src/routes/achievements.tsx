...
export const Route = createFileRoute("/achievements")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Career medals and titles across wrestling and fitness competitions.",
      },
    ],
  }),
        ...
        <div className="mx-auto max-w-4xl px-6 grid grid-cols-3 gap-6 text-center">
          {(["gold", "silver", "bronze"] as const).map((p) => {
            ...
            return (
              <div key={p}>
                <Icon
                  className={`mx-auto h-10 w-10 mb-3 ${p === "gold" ? "text-yellow-400" : p === "silver" ? "text-slate-300" : "text-amber-600"}`}
                />
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {list.map((item, i) => (
              ...
              >
                <div
                  className={`absolute top-0 left-0 right-0 h-1 bg-gradient-to-r ${colors[item.position]}`}
                />
                <div className="flex items-start justify-between mb-6">
                  <div
                    className={`h-14 w-14 rounded-full bg-gradient-to-br ${colors[item.position]} flex items-center justify-center`}
                  >
                ...
                <p
                  className={`mt-4 text-xs uppercase tracking-[0.25em] font-display ${colors[item.position].split(" ").pop()}`}
                >
                  {item.position} medal
                </p>
              </div>
